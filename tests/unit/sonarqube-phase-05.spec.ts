...
test('authenticates through SonarQube when SPA client-side router triggers delayed redirect to /sessions/new', async ({ page }) => {
  const outputDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'sonarqube-spa-login-'));
  const spaLoadingHtml = '<!doctype html><title>SonarQube</title><div id="root">Loading...</div><script>setTimeout(()=>{location.href="https://sonar.example/sessions/new?return_to=%2Fdashboard%3Fid%3Dservice-a";}, 150);</script>';
  const loginHtml = '<!doctype html><title>Log in - SonarQube</title><form action="/sessions/new" method="POST"><input id="login-input" name="login" type="text" /><input id="password-input" name="password" type="password" /><button type="submit">Log in</button></form>';
  const homeHtml = '<!doctype html><title>Service A Overview</title><nav aria-label="Project"><a href="https://sonar.example/dashboard?id=service-a">Overview</a><a href="https://sonar.example/project/issues?id=service-a">Issues</a></nav><header data-component="project-content-header"><h1>Overview</h1><a role="link" href="https://sonar.example/dashboard?id=service-a">service-a</a></header><main><button role="tab" aria-selected="false">Overall Code</button></main><script>document.querySelector("[role=tab]").onclick=()=>location.href="https://sonar.example/dashboard?id=service-a&codeScope=overall";</script>';
  const overallHtml = '<!doctype html><title>Service A Overall</title><header data-component="project-content-header"><a href="https://sonar.example/dashboard?id=service-a">service-a</a></header><nav aria-label="Project"><a href="https://sonar.example/project/issues?id=service-a">Issues</a></nav><main><h1>Overview</h1><div id="tabpanel-overall">overall content</div></main>';
  const issuesHtml = '<!doctype html><title>Service A Issues</title><header data-component="project-content-header"><a href="https://sonar.example/dashboard?id=service-a">service-a</a></header><h1>Issues</h1><nav data-testid="issues-nav-bar" aria-label="Filters"><div data-component="facet-box" data-property="types"><button aria-label="Type" aria-expanded="true"></button><div role="group"><button role="checkbox" data-facet="BUG" aria-label="Bug 1"><span class="name">Bug</span><span class="stat">1</span></button></div></div><div data-component="facet-box" data-property="severities"><button aria-label="Severity" aria-expanded="true"></button><div role="group"><button role="checkbox" data-facet="CRITICAL" title="Critical"><span class="name">Critical</span><span class="stat">4</span></button></div></div></nav>';
...
test('captures SonarQube Enterprise dashboard using Overall tab and panel attribute variants', async ({ page }) => {
  const outputDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'sonarqube-enterprise-'));
  const targetBranchUrl = 'https://sonar.example/dashboard?id=service-a&branch=release%2Fsit';
  const homeHtml = '<!doctype html><title>Service A Overview</title><nav aria-label="Project"><a href="https://sonar.example/dashboard?id=service-a">Overview</a><a href="https://sonar.example/project/issues?id=service-a&branch=release%2Fsit">Issues</a></nav><div class="project-header"><h1>Service A</h1></div><main><div role="tablist"><button role="tab" id="tab-new" aria-selected="true">New Code</button><button role="tab" id="tab-overall" data-value="overall" aria-controls="tabpanel-overall" aria-selected="false">Overall</button></div></main><script>document.getElementById("tab-overall").onclick=()=>location.href="https://sonar.example/dashboard?id=service-a&branch=release%2Fsit&codeScope=overall";</script>';
  const overallHtml = '<!doctype html><title>Service A Overall</title><div class="project-header"><h1>Service A</h1></div><nav aria-label="Project"><a href="https://sonar.example/project/issues?id=service-a&branch=release%2Fsit">Issues</a></nav><main><div role="tablist"><button role="tab" id="tab-overall" aria-selected="true">Overall</button></div><div id="tabpanel-overall" role="tabpanel">overall measures content</div></main>';
  const issuesHtml = '<!doctype html><title>Service A Issues</title><div class="project-header"><h1>Service A</h1></div><h1>Issues</h1><nav data-testid="issues-nav-bar" aria-label="Filters"><div data-component="facet-box" data-property="types"><button aria-label="Type" aria-expanded="true"></button><div role="group"><button role="checkbox" data-facet="BUG" aria-label="Bug 1"><span class="name">Bug</span><span class="stat">1</span></button></div></div><div data-component="facet-box" data-property="severities"><button aria-label="Severity" aria-expanded="true"></button><div role="group"><button role="checkbox" data-facet="CRITICAL" title="Critical"><span class="name">Critical</span><span class="stat">1</span></button></div></div></nav>';
