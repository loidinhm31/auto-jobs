...
export function parseReportServerArgs(argv: readonly string[] = [], env: NodeJS.ProcessEnv = process.env): ReportServerConfig {
  ...
  return { root: path.resolve(root), host, port, allowLan, help };
}
...
function usage(): void {
  ...
  console.log('Use --host 0.0.0.0 --allow-lan only on a trusted network; it binds all IPv4 interfaces and has no authentication.');
}
...
export async function main(argv: readonly string[] = process.argv.slice(2), env: NodeJS.ProcessEnv = process.env): Promise<void> {
  ...
  const handle = await createReportServer(config.root, { host: config.host, port: config.port, allowLan: config.allowLan });
