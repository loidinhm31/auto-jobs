...
function projectCard(project: AggregateProjectSummary): string {
  ...
  const runs = project.runs.length === 0
    ? '<p class="empty-state">No validated historical run manifest with a build identity.</p>'
    : `<div class="table-scroll"><table class="compact-table"><caption>Historical runs for ${escapeHtmlText(project.name)}</caption><thead><tr><th scope="col">Build</th><th scope="col">Run</th><th scope="col">State</th><th scope="col">Artifacts</th></tr></thead><tbody>${project.runs.map((run) => `<tr><th scope="row">#${run.buildNumber}</th><td><code>${escapeHtmlText(run.runId)}</code></td><td>${stateBadge(run.state)}</td><td>${runLinks(run)}${run.warnings.length === 0 ? '' : `<ul class="inline-warnings">${run.warnings.map((warning) => `<li>${escapeHtmlText(warning)}</li>`).join('')}</ul>`}</td></tr>`).join('')}</tbody></table></div>`;
...
<body>
  ...
  <footer><p>Aggregate history is built only from validated schema-v2 manifests. Missing or invalid entries are omitted and surfaced as warnings.</p></footer>
</body>
</html>
`;
}
