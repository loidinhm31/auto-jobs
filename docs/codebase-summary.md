...
the current implementation and verification boundary through Phase 05 dynamic
credentials and Phase 01 active-configuration persistence in the Control Dashboard;
it does not claim a live Jenkins run.
...
- Repomix inventory: 262 repository files packed; binary and ignored files stay
  outside the summary. The XML captures current source, tests, fixtures, plans,
  and documentation.
- Compaction metrics: 5,151,655 characters and 1,494,492 tokens.
...
| `src/reporting/control-page/hooks/` | Headless React hooks: `useControlApi`, `useConfigManager` (active configuration persistence), `useCredentialsManager`, `useBrowserSettings`, and `useRunPoller`. |
