...
the current implementation boundary through Phase 04 control UI and dynamic-
credentials Phases 01–04; it does not claim a live Jenkins run.
...
- Repomix inventory: 193 repository files were packed; binary/ignored files
  remain outside the summary. The current XML compaction contains the Phase 04
  source, tests, fixtures, plans, and documentation.
- Compaction metrics: 4,678,660 characters and 1,380,637 tokens.
...
### Control UI credential workflow (Phase 04)
...
URLs, page HTML after save/close, or API success/error payloads. The E2E
contract in `tests/e2e/control-page.spec.ts` covers modal accessibility,
presence transitions, save/clear/reopen state, input wiping, and no test
secret in page HTML.
...
redaction with injected executors.
The Phase 04 control-page E2E gate separately proves credential-modal
accessibility and zero-leakage UI transitions against an isolated server.
