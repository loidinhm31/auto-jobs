...
the current implementation boundary through Phase 3 and Phase 01 of the
dynamic-credentials plan; it does not claim a live Jenkins run.
...
- Phase 01 addition: a canonical, local `SecretStore` backend for validated
  ...
  mode but not yet exposed through an HTTP secrets API or run-environment
  injection.
- Repomix inventory: 188 repository files were packed; binary/ignored files
  remain outside the summary. The current XML compaction contains the Phase 01
  source, tests, fixtures, plans, and documentation.
- Compaction metrics: 4,588,846 characters and 1,359,247 tokens.
...
`/^[A-Za-z_][A-Za-z0-9_]{0,127}$/`; missing or empty files return `{}`.
Malformed JSON, arrays/null, non-string values, invalid keys, oversized files,
non-regular files, and symlinks are rejected. `readSecrets()` returns a frozen
map; `listSecretNames()` returns sorted frozen names. `putSecret`,
`putSecrets`, `deleteSecret`, and `deleteSecrets` validate names and use a
latest-file read-modify-write inside one in-memory lock.
...
on `ControlRouterContext`. The current router does not dispatch `/api/secrets`,
and execution still uses caller-provided environment values. API exposure and
...
| `src/reporting/report-server-constants.ts` | Shared report/control limits plus `SECRETS_FILE_NAME` and `MAX_SECRET_FILE_BYTES`. |
| `src/reporting/report-server-control.ts` | Control routing and `ControlRouterContext`, including the optional SecretStore dependency. |
...
- Keep runtime credentials in the environment or a trusted CI secret store;
  local Phase 01 values belong only in git-ignored
  `config/secrets.local.json`. Never commit values, cookies, tokens, or
  credential-bearing URLs.
- Keep SecretStore keys environment-style and payloads bounded. Use frozen
  snapshots, sorted deterministic JSON, latest-file read-modify-write locking,
  exclusive temporary files, sync/close/rename, and rename-failure cleanup.
  Write/sync errors close handles; POSIX `0o600` mode is advisory on Windows,
  so config-directory ACLs are required.
